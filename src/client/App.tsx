...
function Admin() {
  ...
  const editResource = (item: Resource) => { setEditingResourceId(item.id); setResourceForm({ title: item.title, description: item.description, category: item.category, url: item.url, icon: item.icon }); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const editWork = (item: Work) => { setEditingWorkId(item.id); setWorkForm({ title: item.title, description: item.description, url: item.url, imageUrl: item.imageUrl }); window.scrollTo({ top: 0, behavior: "smooth" }); };

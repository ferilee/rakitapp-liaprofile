...
function PublicSite() {
  ...
  return <div className="site-shell min-h-screen overflow-hidden bg-[#07131d] text-slate-100">
    ...
    <header className="relative z-30 mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8">
      ...
      <nav className={`${menuOpen ? "flex" : "hidden"} absolute left-5 right-5 top-20 flex-col gap-2 rounded-2xl border border-white/10 bg-[#0b1c29]/95 p-3 shadow-2xl backdrop-blur-xl sm:static sm:flex sm:flex-row sm:items-center sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none`}>
...
function AdminLogin({ value, error, onChange, onSubmit }: { value: string; error: string; onChange: (value: string) => void; onSubmit: (event: React.FormEvent) => void }) {
  return <div className="grid min-h-screen place-items-center bg-[#07131d] px-5 text-slate-100"><Card className="w-full max-w-md"><CardHeader><span className="mb-5 grid h-12 w-12 place-items-center rounded-2xl bg-cyan-300 text-slate-950"><FolderOpen size={22} /></span><CardTitle>Masuk ke ruang pengelola</CardTitle><CardDescription>Masukkan token admin dari environment aplikasi untuk mengelola isi profil.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={onSubmit}><Input required type="password" placeholder="Token admin" value={value} onChange={(event) => onChange(event.target.value)} />{error && <p className="text-sm text-rose-300">{error}</p>}<Button className="w-full" type="submit">Masuk ke dashboard</Button><a href="/" className="block text-center text-sm text-slate-500 hover:text-cyan-200">Kembali ke situs publik</a></form></CardContent></Card></div>;
}
...
function Status({ icon: Icon, label }: { icon: LucideIcon; label: string }) { return <div className="flex items-center gap-3 rounded-2xl border border-white/5 bg-white/[0.03] p-4"><Icon size={17} className="text-cyan-200" /><span className="text-sm text-slate-300">{label}</span><Check size={15} className="ml-auto text-emerald-300" /></div>; }

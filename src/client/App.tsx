...
import { getSite, adminRequest, uploadAdminAsset, type Fact, type Profile, type Resource, type SiteData, type Social, type Work } from "./api";
...
function Admin() {
  const [data, setData] = useState<{ profile: SiteData["profile"]; resources: Resource[]; works: Work[]; facts: Fact[]; socials: Social[] } | null>(null);
  const [message, setMessage] = useState("");
  const [token, setToken] = useState(() => localStorage.getItem("lia-admin-token") ?? "");
  const [tokenInput, setTokenInput] = useState("");
  const [loadError, setLoadError] = useState("");
  const [adminDark, setAdminDark] = useState(() => localStorage.getItem("lia-theme") === "dark");
  const [resourceForm, setResourceForm] = useState({ title: "", description: "", category: "Belajar", url: "", icon: "book-open", parentId: "" });
  ...
  type AdminTab = "profile" | "resource-add" | "work-add" | "resource-list" | "work-list";
  const [adminTab, setAdminTab] = useState<AdminTab>("resource-add");
  ...
  const saveResource = async (event: React.FormEvent) => { event.preventDefault(); const editing = editingResourceId !== null; await adminRequest(`/api/admin/resources${editing ? `/${editingResourceId}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify({ ...resourceForm, parentId: resourceForm.parentId ? Number(resourceForm.parentId) : null }) }); setResourceForm({ title: "", description: "", category: "Belajar", url: "", icon: "book-open", parentId: "" }); setEditingResourceId(null); setMessage(editing ? "Sumber belajar diperbarui." : "Sumber belajar ditambahkan."); load(); };
  const saveWork = async (event: React.FormEvent) => { event.preventDefault(); const editing = editingWorkId !== null; await adminRequest(`/api/admin/works${editing ? `/${editingWorkId}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify(workForm) }); setWorkForm({ title: "", description: "", url: "", imageUrl: "" }); setEditingWorkId(null); setMessage(editing ? "Karya diperbarui." : "Karya ditambahkan."); load(); };
  const editResource = (item: Resource) => { setEditingResourceId(item.id); setResourceForm({ title: item.title, description: item.description, category: item.category, url: item.url, icon: item.icon, parentId: item.parentId?.toString() ?? "" }); setAdminTab("resource-add"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const editWork = (item: Work) => { setEditingWorkId(item.id); setWorkForm({ title: item.title, description: item.description, url: item.url, imageUrl: item.imageUrl }); setAdminTab("work-add"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const cancelResourceEdit = () => { setEditingResourceId(null); setResourceForm({ title: "", description: "", category: "Belajar", url: "", icon: "book-open", parentId: "" }); };
  const cancelWorkEdit = () => { setEditingWorkId(null); setWorkForm({ title: "", description: "", url: "", imageUrl: "" }); };
  ...
  const toggleAdminTheme = () => { setAdminDark((value) => { const next = !value; localStorage.setItem("lia-theme", next ? "dark" : "light"); return next; }); };
  if (!token || loadError) return <AdminLogin dark={adminDark} onToggleTheme={toggleAdminTheme} value={tokenInput} error={loadError} onChange={setTokenInput} onSubmit={signIn} />;
  if (!data) return <LoadingState />;
  return <div className={`admin-shell${adminDark ? " admin-theme-dark" : ""}`} data-admin-tab={adminTab}><div className="admin-content"><div className="admin-header"><div><a href="/" className="admin-back-link">← Kembali ke situs</a><h1>Ruang pengelola</h1><p>Kelola tautan belajar dan karya untuk Lia Physics Hub.</p></div><div className="admin-header-actions"><button type="button" className="admin-theme-toggle" onClick={toggleAdminTheme} aria-label="Ganti tema">{adminDark ? <Sun size={16} /> : <Moon size={16} />}</button><Badge><span className="mr-2 h-1.5 w-1.5 rounded-full bg-emerald-400" /> Terhubung ke SQLite</Badge></div></div>{message && <div className="admin-message"><Check size={16} /> {message}</div>}<ProfileSettings profile={data.profile} socials={data.socials} onSaved={(text) => { setMessage(text); load(); }} /><div className="admin-forms grid gap-6 lg:grid-cols-2"><div className="admin-resource-panel"><Card><CardHeader><CardTitle>{editingResourceId !== null ? "Edit sumber belajar" : "Tambah sumber belajar"}</CardTitle><CardDescription>Tambahkan kartu menuju Drive, YouTube, Quizizz, atau platform lain.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={saveResource}><Input required placeholder="Judul sumber belajar" value={resourceForm.title} onChange={(e) => setResourceForm({ ...resourceForm, title: e.target.value })} /><Textarea required placeholder="Deskripsi singkat" value={resourceForm.description} onChange={(e) => setResourceForm({ ...resourceForm, description: e.target.value })} /><div className="grid gap-4 sm:grid-cols-2"><Input placeholder="Kategori" value={resourceForm.category} onChange={(e) => setResourceForm({ ...resourceForm, category: e.target.value })} /><select className="admin-select" value={resourceForm.parentId} onChange={(e) => setResourceForm({ ...resourceForm, parentId: e.target.value })} aria-label="Sumber utama"><option value="">Sumber utama</option>{data.resources.filter((item) => item.parentId === null && item.id !== editingResourceId).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><Input required type="url" placeholder="https://..." value={resourceForm.url} onChange={(e) => setResourceForm({ ...resourceForm, url: e.target.value })} /></div><div className="flex flex-wrap gap-2"><Button type="submit"><Clipboard size={16} /> {editingResourceId !== null ? "Simpan perubahan" : "Simpan sumber"}</Button>{editingResourceId !== null && <Button type="button" variant="ghost" onClick={cancelResourceEdit}>Batal</Button>}</div></form></CardContent></Card></div><div className="admin-work-panel"><Card><CardHeader><CardTitle>{editingWorkId !== null ? "Edit karya" : "Tambah karya"}</CardTitle><CardDescription>Tampilkan praktik baik, media, atau proyek siswa.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={saveWork}><Input required placeholder="Judul karya" value={workForm.title} onChange={(e) => setWorkForm({ ...workForm, title: e.target.value })} /><Textarea required placeholder="Deskripsi singkat" value={workForm.description} onChange={(e) => setWorkForm({ ...workForm, description: e.target.value })} /><Input required type="url" placeholder="URL karya" value={workForm.url} onChange={(e) => setWorkForm({ ...workForm, url: e.target.value })} /><Input type="url" placeholder="URL thumbnail (opsional)" value={workForm.imageUrl} onChange={(e) => setWorkForm({ ...workForm, imageUrl: e.target.value })} /><div className="flex flex-wrap gap-2"><Button type="submit"><Clipboard size={16} /> {editingWorkId !== null ? "Simpan perubahan" : "Simpan karya"}</Button>{editingWorkId !== null && <Button type="button" variant="ghost" onClick={cancelWorkEdit}>Batal</Button>}</div></form></CardContent></Card></div></div><div className="admin-lists mt-8 grid gap-6 lg:grid-cols-2"><div className="admin-resource-list-panel"><AdminList title="Sumber belajar" items={data.resources.map((item) => ({ id: item.id, title: item.title, meta: item.category }))} onEdit={(id) => editResource(data.resources.find((item) => item.id === id)!)} onDelete={(id) => remove("resources", id)} /></div><div className="admin-work-list-panel"><AdminList title="Karya & inovasi" items={data.works.map((item) => ({ id: item.id, title: item.title, meta: "Karya" }))} onEdit={(id) => editWork(data.works.find((item) => item.id === id)!)} onDelete={(id) => remove("works", id)} /></div></div><AdminMobileNav active={adminTab} onChange={setAdminTab} /></div></div>;
}

function AdminLogin({ dark, onToggleTheme, value, error, onChange, onSubmit }: { dark: boolean; onToggleTheme: () => void; value: string; error: string; onChange: (value: string) => void; onSubmit: (event: React.FormEvent) => void }) {
  return <div className={`admin-shell admin-auth${dark ? " admin-theme-dark" : ""}`}><button type="button" className="admin-theme-toggle admin-login-theme-toggle" onClick={onToggleTheme} aria-label="Ganti tema">{dark ? <Sun size={16} /> : <Moon size={16} />}</button><Card className="w-full max-w-md"><CardHeader><span className="mb-5 grid h-12 w-12 place-items-center rounded-2xl bg-cyan-300 text-slate-950"><FolderOpen size={22} /></span><CardTitle>Masuk ke ruang pengelola</CardTitle><CardDescription>Masukkan token admin dari environment aplikasi untuk mengelola isi profil.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={onSubmit}><Input required type="password" placeholder="Token admin" value={value} onChange={(event) => onChange(event.target.value)} />{error && <p className="text-sm text-rose-300">{error}</p>}<Button className="w-full" type="submit">Masuk ke dashboard</Button><a href="/" className="admin-back-link admin-back-center">Kembali ke situs publik</a></form></CardContent></Card></div>;
}

function ProfileSettings({ profile, socials, onSaved }: { profile: Profile; socials: Social[]; onSaved: (message: string) => void }) {
  const [form, setForm] = useState({ name: profile.name, role: profile.role, school: profile.school, tagline: profile.tagline, bio: profile.bio, avatarUrl: profile.avatarUrl, whatsapp: profile.whatsapp, email: profile.email });
  const [socialForms, setSocialForms] = useState<Record<number, { label: string; url: string }>>({});
  const [uploading, setUploading] = useState(false);
  const [notice, setNotice] = useState("");

  useEffect(() => {
    setForm({ name: profile.name, role: profile.role, school: profile.school, tagline: profile.tagline, bio: profile.bio, avatarUrl: profile.avatarUrl, whatsapp: profile.whatsapp, email: profile.email });
    const next: Record<number, { label: string; url: string }> = {};
    socials.forEach((social) => { next[social.id] = { label: social.label, url: social.url }; });
    setSocialForms(next);
  }, [profile, socials]);

  const saveProfile = async (event: React.FormEvent) => { event.preventDefault(); await adminRequest("/api/admin/profile", { method: "PATCH", body: JSON.stringify(form) }); setNotice("Profil berhasil diperbarui."); onSaved("Profil berhasil diperbarui."); };
  const uploadAvatar = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setUploading(true); setNotice(""); try { const result = await uploadAdminAsset(file); setForm((current) => ({ ...current, avatarUrl: result.url })); setNotice("Foto berhasil diunggah. Simpan profil untuk menerapkannya."); } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Upload foto gagal."); } finally { setUploading(false); } };
  const saveSocial = async (social: Social) => { const values = socialForms[social.id]; if (!values) return; await adminRequest(`/api/admin/socials/${social.id}`, { method: "PATCH", body: JSON.stringify(values) }); setNotice(`${social.label} berhasil diperbarui.`); onSaved(`${social.label} berhasil diperbarui.`); };

  return <section className="admin-profile-settings"><Card><CardHeader><CardTitle>Profil & media sosial</CardTitle><CardDescription>Ganti foto profil dan perbarui link yang tampil di halaman publik.</CardDescription></CardHeader><CardContent><form className="admin-profile-form" onSubmit={saveProfile}><div className="admin-profile-intro"><img className="admin-avatar-preview" src={form.avatarUrl} alt={form.name} /><div><label className="admin-field-label">Foto profil</label><Input type="file" accept="image/*" onChange={uploadAvatar} disabled={uploading} /><p className="admin-field-help">{uploading ? "Mengunggah ke RustFS..." : "PNG, JPG, atau WEBP maksimal 5 MB."}</p><Input type="url" placeholder="Atau tempel URL foto" value={form.avatarUrl} onChange={(event) => setForm({ ...form, avatarUrl: event.target.value })} /></div></div><div className="admin-profile-grid"><label><span className="admin-field-label">Nama</span><Input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label><span className="admin-field-label">Peran</span><Input required value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })} /></label><label><span className="admin-field-label">Sekolah</span><Input required value={form.school} onChange={(event) => setForm({ ...form, school: event.target.value })} /></label><label><span className="admin-field-label">WhatsApp</span><Input required value={form.whatsapp} onChange={(event) => setForm({ ...form, whatsapp: event.target.value })} /></label><label><span className="admin-field-label">Email</span><Input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label className="admin-profile-wide"><span className="admin-field-label">Tagline</span><Input required value={form.tagline} onChange={(event) => setForm({ ...form, tagline: event.target.value })} /></label><label className="admin-profile-wide"><span className="admin-field-label">Bio singkat</span><Textarea required value={form.bio} onChange={(event) => setForm({ ...form, bio: event.target.value })} /></label></div><div className="admin-profile-actions"><Button type="submit"><Check size={16} /> Simpan profil</Button>{notice && <span className="admin-field-help">{notice}</span>}</div></form><div className="admin-social-settings"><div className="admin-subheading"><p>Link media sosial</p><span>Perubahan langsung dipakai di halaman publik.</span></div><div className="admin-social-grid">{socials.map((social) => <div key={social.id} className="admin-social-row"><div className="admin-social-name"><span className="admin-social-dot" />{social.label}</div><Input value={socialForms[social.id]?.label ?? ""} aria-label={`Label ${social.label}`} onChange={(event) => setSocialForms((current) => ({ ...current, [social.id]: { ...current[social.id], label: event.target.value } }))} /><Input type="url" value={socialForms[social.id]?.url ?? ""} aria-label={`URL ${social.label}`} onChange={(event) => setSocialForms((current) => ({ ...current, [social.id]: { ...current[social.id], url: event.target.value } }))} /><Button type="button" variant="secondary" size="sm" onClick={() => saveSocial(social)}>Simpan</Button></div>)}</div></div></CardContent></Card></section>;
}

function AdminMobileNav({ active, onChange }: { active: "profile" | "resource-add" | "work-add" | "resource-list" | "work-list"; onChange: (tab: "profile" | "resource-add" | "work-add" | "resource-list" | "work-list") => void }) {
  const items = [
    { id: "profile" as const, label: "Profil", icon: UserRound },
    { id: "resource-add" as const, label: "Tambah sumber", icon: Plus },
    { id: "work-add" as const, label: "Tambah karya", icon: FlaskConical },
    { id: "resource-list" as const, label: "Sumber belajar", icon: BookOpen },
    { id: "work-list" as const, label: "Karya & inovasi", icon: NotebookPen },
  ];
  return <nav className="admin-mobile-nav" aria-label="Navigasi admin">{items.map(({ id, label, icon: Icon }) => <button key={id} type="button" data-active={active === id} onClick={() => { onChange(id); window.scrollTo({ top: 0, behavior: "smooth" }); }}><Icon size={18} /><span>{label}</span></button>)}</nav>;
}

function AdminList({ title, items, onEdit, onDelete }: { title: string; items: { id: number; title: string; meta: string }[]; onEdit: (id: number) => void; onDelete: (id: number) => void }) { return <Card><CardHeader><CardTitle>{title} <span className="ml-2 text-sm font-normal text-slate-500">{items.length}</span></CardTitle></CardHeader><CardContent><div className="space-y-2">{items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-slate-950/20 px-3 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{item.title}</p><p className="text-xs text-slate-500">{item.meta}</p></div><div className="flex items-center gap-1"><Button variant="ghost" size="icon" onClick={() => onEdit(item.id)} aria-label={`Edit ${item.title}`}><Pencil size={15} className="text-cyan-600" /></Button><Button variant="ghost" size="icon" onClick={() => onDelete(item.id)} aria-label={`Hapus ${item.title}`}><Trash2 size={15} className="text-rose-300" /></Button></div></div>)}</div></CardContent></Card>; }
export { App };

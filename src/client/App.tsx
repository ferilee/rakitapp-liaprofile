...
function Admin() {
  ...
  const saveResource = async (event: React.FormEvent) => { event.preventDefault(); const editing = editingResourceId !== null; await adminRequest(`/api/admin/resources${editing ? `/${editingResourceId}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify(resourceForm) }); setResourceForm({ title: "", description: "", category: "Belajar", url: "", icon: "book-open" }); setEditingResourceId(null); setMessage(editing ? "Sumber belajar diperbarui." : "Sumber belajar ditambahkan."); load(); };
  ...
  const editResource = (item: Resource) => { setEditingResourceId(item.id); setResourceForm({ title: item.title, description: item.description, category: item.category, url: item.url, icon: item.icon }); setAdminTab("resource-add"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const cancelResourceEdit = () => { setEditingResourceId(null); setResourceForm({ title: "", description: "", category: "Belajar", url: "", icon: "book-open" }); };
